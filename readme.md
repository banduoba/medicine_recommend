# 毕业设计--基于Django的书籍推荐系统和论坛

## 说明

1. 新手建议结合pycharm使用，https://www.jetbrains.com/pycharm/，下载专业版试用30天。
2. 注册普通用户通过web界面来设置，创建创建用户通过creeatsuperuser创建。下文有详细命令
3. 导入书籍信息通过insert_movies_script.py来操作 (会删除已有的所有信息!)
4. 前端展示 浏览最多，评分最多，收藏最多，写的比较直白，你可以改的委婉点: 最热书籍，火爆排行...之类的。每种有10条。
...
主要功能: 录入图书信息，用户打分，书籍标签分类，书籍推荐，书籍分享，书籍收藏，后台管理系统。
...
所有未打分的书籍中按照浏览数降序选一部分填充进去。

### 基于用户的推荐

1. 用户需要给书籍打分。通过用户已打分的部分来计算相似度,如果用户未打分，或者没有其他用户，则按照浏览数降序返回。
2. 通过pearson算法来计算用户之间的距离，找到距离最近的N个用户。将这些用户中已打分的书籍(且要推荐的用户未看过的部分)返回。
...
9. populate_movies_script.py 填充书籍数据到数据库中
